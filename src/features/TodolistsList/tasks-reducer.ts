    ...
    todolistsAPI.createTask(todolistId, title)
        .then(res => {
            if (res.data.resultCode === ResultCode.OK) {
                // res.data. - в эту дату нам помещает сам axios то что возвращает бэк при запросе
        ...
        .catch((error: AxiosError<ErrorsType>) => {
...
export const updateTaskTC = (taskId: string, domainModel: UpdateDomainTaskModelType, todolistId: string) =>
    async (dispatch: Dispatch<ActionsType>, getState: () => AppRootStateType) => {
        ...
        try {
            ...
            if (axios.isAxiosError<ErrorsType>(error)) {
                const errorMessage = error.response ? error.response.data.message : error.message
                handleServerNetworkError(dispatch, error.message)
            } else {
                const error_2 = (error as Error).message
            }
        }
